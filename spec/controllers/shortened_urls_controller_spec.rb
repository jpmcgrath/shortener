  ...
  describe "#show" do
    ...
    context "valid keys" do
      context "real key" do
        ...
        context "when its a rails request" do
          let(:run_action!) do
            if Rails::VERSION::MAJOR < 5
              get :show, { id: key }
            else
          ...
          context "Shortener.ignore_robots == true" do
            it "calls fetch with token with track argument as false" do
              ...
              expect(Shortener::ShortenedUrl).to receive(:fetch_with_token)
                                                   .with(hash_including(track: false))
                                                   .and_call_original
              run_action!
            end
          end
          context "Shortener.ignore_robots == false (default)" do
            it "calls fetch with token with track argument as true" do
              ...
              expect(Shortener::ShortenedUrl).to receive(:fetch_with_token)
                                                   .with(hash_including(track: true))
                                                   .and_call_original
              run_action!
            end
          end
          context "bot user agent" do
            ...
            it "Shortener.ignore_robots == true" do
              Shortener.ignore_robots = true
              expect(Shortener::ShortenedUrl).to receive(:fetch_with_token)
                                                   .with(hash_including(track: false))
                                                   .and_call_original
            ...
            it "Shortener.ignore_robots == false (default)" do
              Shortener.ignore_robots = false
              expect(Shortener::ShortenedUrl).to receive(:fetch_with_token)
                                                   .with(hash_including(track: true))
                                                   .and_call_original
          ...
          context "non-bot user agent" do
            before do
              request.env["HTTP_USER_AGENT"] =
                "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.111 Safari/537.36"
            end
            it "call fetch with token with ignore robots as true should track" do
              Shortener.ignore_robots = true
              expect(Shortener::ShortenedUrl).to receive(:fetch_with_token).with(hash_including(track: true)).and_call_original
              run_action!
            end
            it "call fetch with token with ignore robots as false should track" do
              Shortener.ignore_robots = false
              expect(Shortener::ShortenedUrl).to receive(:fetch_with_token).with(hash_including(track: true)).and_call_original
              run_action!
            end
