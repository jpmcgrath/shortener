class Shortener::ShortenedUrl < ActiveRecord::Base
  ...
  def self.generate!(destination_url, params={})
    params = Shortener.adhoc_shortening_params
             .merge(Shortener.default_shortening_params || {})
             .merge(params)
             .except(:url_options)
    do_generate!(destination_url, params)
  end

  # return shortened url on success, nil on failure
  def self.generate(destination_url, params={})
    begin
      generate!(destination_url, params)
    rescue => e
      logger.info e
      nil
    end
  ...
  def self.do_generate!(destination_url, params)
    # if we get a shortened_url object with a different owner, generate
    # new one for the new owner. Otherwise return same object
    result = if destination_url.is_a? Shortener::ShortenedUrl
      if destination_url.owner == params[:owner]
        destination_url
      else
        generate!(destination_url.url, params)
      end
    else
      scope = params[:owner] ? params[:owner].shortened_urls : self
      creation_method = params[:fresh] ? 'create' : 'first_or_create'
      scope.where(url: clean_url(destination_url))
        .send(creation_method,
              unique_key: params[:custom_key],
